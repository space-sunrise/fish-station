...
5. **EMP на маске (`EmpDisabled`):** для обычной маски СБ отправка полностью блокируется, пока EMP-disabled активен.
6. **После EMP (MalfunctionUntil, по умолчанию 60 с):** на обычной маске СБ можно слать, но без локации и с помехами в названии.
6a. **Спецмаски ЦК/ОБР/ДСО** имеют `EmpResistance` (`strengthMultiplier: 0`, паттерн гарнитуры ниндзя): EMP attempt отменяется → нет `EmpDisabled`, нет soft-malfunction; доклад идёт как обычно. Обычная `ClothingMaskGasSecurity` **без** этого компонента.
6b. **Спецгарнитуры** ЦК/ОБР/ДСО/SpecOps — тот же `EmpResistance`; станционные гарнитуры СБ/Command/BlueShield **не** менялись.
...
10. **Уничтожение telecom-серверов** влияет на обычные (не LongRange) каналы штатным RadioSystem; система доклада не обходит телеком отдельно. Каналы CentCom/ERT/DeathSquad уже `longRange: true`.
11. **EMP-устойчивость спецподразделений** реализована **только** через существующий `EmpResistance` на спецмасках/спецгарнитурах — без правок EmpSystem/RadioSystem. `TelecomExempt` для EMP не используется (он про telecom server, не про EMP).
...
15. **BlueShield** — станционная роль (`BlueShield` канал `longRange: false`); EMP-hardening на их гарнитуру/маску не добавлялся.
